...
void AddressWidget::showAddEntryDialog()
{
    ...
    if (aDialog.exec())
        addEntry(aDialog.name(), aDialog.address(), aDialog.phone());
}
//! [2]

//! [3]
void AddressWidget::addEntry(const QString &name, const QString &address, const QString &phone)
{
    if (!table->getContacts().contains({ name, address, phone })) {
...
//! [4b]
    ...
    aDialog.editAddress(name, address, phone);
...
void AddressWidget::setupTabs()
{
    ...
    const auto groups = { "AEIOUY"_L1, "BCDFGHJKLMNPQRSTVWXZ"_L1};
...
void AddressWidget::readFromFile()
{
    ...
    if (contacts.isEmpty()) {
        ...
        for (const auto &contact: std::as_const(contacts))
            addEntry(contact.name, contact.address, contact.phone);
    }
}
//! [7]
